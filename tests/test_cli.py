import argparse
import json
from forge.cli import cmd_ai, cmd_exec, cmd_completion, cmd_status, cmd_health, cmd_config_path, cmd_scan, cmd_init, cmd_share, cmd_notes, cmd_feature, cmd_doctor, cmd_pr, cmd_graph, cmd_log
...
class TestCmdConfigPath:
    def test_prints_config_path(self, forge_config, captured_print):
        cmd_config_path(_ns(sub="path", fix=False))
        assert len(captured_print) == 1
        assert captured_print[0].endswith("config.json")

    def test_config_validate_empty(self, forge_config, captured_print):
        cmd_config_path(_ns(sub="validate", fix=False))
        output = " ".join(captured_print)
        assert "valid" in output.lower()

    def test_config_validate_with_issues(self, forge_config, captured_print):
...
class TestCmdCompletion:
    def test_bash_completion(self, captured_print):
        ...
        output = " ".join(captured_print)
        assert "_forge_completions" in output
        assert "complete -F _forge_completions forge" in output

    def test_zsh_completion(self, captured_print):
        cmd_completion(_ns(shell="zsh"))
        output = " ".join(captured_print)
        assert "#compdef forge" in output
...
class TestCmdInit:
    def test_prints_init_message(self, forge_config, captured_print):
...
class TestCmdScan:
    def test_scan_empty(self, forge_config, captured_print):
        cmd_scan(_ns())
        output = " ".join(captured_print)
...
class TestCmdStatus:
    def test_status_empty(self, forge_config, captured_print):
        cmd_status(_ns(name=None, json=False, graph=False))
        output = " ".join(captured_print)
        assert "Workspace" in output
        assert "0 total" in output

    def test_status_json(self, forge_config, captured_print):
    ...
    def test_status_with_graph(self, forge_config, populated_config, captured_print, mocker):
        mocker.patch("forge.engine.get_overall_status", return_value={
        ...
        mocker.patch("forge.graph.cross_repo_impact", return_value=[
...
class TestCmdShare:
    def test_share_note(self, forge_config, captured_print):
        cmd_share(_ns(content="hello world", group="test-group", label="test-label"))
        output = " ".join(captured_print)
        assert "Shared note" in output

    def test_share_default_group(self, forge_config, captured_print):
...
class TestCmdNotes:
    def test_notes_empty_group(self, forge_config, captured_print):
        cmd_notes(_ns(group="empty-group"))
        output = " ".join(captured_print)
        assert "not found" in output or "No notes" in output

    def test_notes_with_content(self, forge_config, captured_print):
...
class TestCmdDoctor:
    def test_doctor_empty(self, forge_config, captured_print):
        cmd_doctor(_ns(json=False))
        output = " ".join(captured_print)
        assert "healthy" in output or "issue" in output

    def test_doctor_json(self, forge_config, captured_print):
...
class TestCmdLog:
    def test_log_empty(self, forge_config, captured_print):
        cmd_log(_ns(name=None, limit=10, json=False))
        output = " ".join(captured_print)
        assert "No sessions" in output

    def test_log_json_empty(self, forge_config, captured_print):
        cmd_log(_ns(name=None, limit=10, json=True))
        import json as j
        data = j.loads(captured_print[0])
        assert data == []

    def test_log_with_session(self, forge_config, captured_print):
        c = forge_config.load_config()
    ...
    def test_log_session_detail(self, forge_config, captured_print, mocker):
        c = forge_config.load_config()
        c.setdefault("sessions", []).append({
            "id": "sess-001", "agent": "claude-code", "feature": "feat-1",
            "started": "2025-01-01T00:00:00", "context": "fix auth bug",
        ...
        forge_config.save_config(c)
        mocker.patch("forge.engine.get_session", return_value={
    ...
    def test_log_session_not_found(self, forge_config, captured_print):
        cmd_log(_ns(name="no-such-session", limit=10, json=False))
        output = " ".join(captured_print)
        assert "not found" in output

    def test_log_session_detail_json(self, forge_config, captured_print, mocker):
        mocker.patch("forge.engine.get_session", return_value={
...
class TestCmdGraph:
    def test_graph_repo_not_found(self, forge_config, captured_print):
        cmd_graph(_ns(name="no-such", type="co-change", format="text", depth=50))
        output = " ".join(captured_print)
        assert "not found" in output

    def test_graph_prints_results(self, forge_config, captured_print, mocker):
        mocker.patch("forge.graph.generate_graph", return_value={
            "repo": "my-repo",
    ...
    def test_graph_json(self, forge_config, captured_print, mocker):
        mocker.patch("forge.graph.generate_graph", return_value={
            "repo": "my-repo",
            "nodes": [],
            "edges": [],
    ...
    def test_graph_branches_text(self, forge_config, captured_print, mocker):
        mocker.patch("forge.graph.generate_graph", return_value={
            "repo": "my-repo",
...
class TestCmdPr:
    def test_pr_create_feature_not_found(self, forge_config, captured_print):
        cmd_pr(_ns(action="create", name="no-such", title=None, body=None, draft=False))
        output = " ".join(captured_print)
        assert "not found" in output

    def test_pr_create_no_repos(self, forge_config, captured_print):
    ...
    def test_pr_create_prints_results(self, forge_config, captured_print, mocker):
        mocker.patch("forge.engine.create_prs", return_value={
            "feature": "my-feat",
            "id": "feat-123",
            "prs": [
...
class TestCmdAiDetect:
    def test_detect_json(self, forge_config, captured_print):
        cmd_ai(_ns(action="detect", json=True, model="", prompt="", backend="", key=None, value=None))
        output = " ".join(captured_print)
        assert "cpu" in output
        assert "memory" in output

    def test_detect_text(self, forge_config, captured_print):
...
class TestCmdAiConfig:
    def test_config_show(self, forge_config, captured_print):
        c = forge_config.load_config()
    ...
    def test_config_set(self, forge_config, captured_print):
