import subprocess
import pytest
from forge import graph as forgegraph


class TestCoChangeGraph:
    def test_nonexistent_path(self):
        result = forgegraph.co_change_graph("/nonexistent/path")
        assert "error" in result

    def test_not_git_repo(self, tmp_path):
        result = forgegraph.co_change_graph(str(tmp_path))
        assert "error" in result

    def test_empty_repo(self, tmp_git_repo):
        result = forgegraph.co_change_graph(str(tmp_git_repo))
    ...
    def test_single_commit(self, tmp_git_repo):
        ...
        result = forgegraph.co_change_graph(str(tmp_git_repo), depth=10)
...
class TestBranchGraph:
    def test_nonexistent_path(self):
        result = forgegraph.branch_graph("/nonexistent")
        assert "error" in result

    def test_not_git(self, tmp_path):
        result = forgegraph.branch_graph(str(tmp_path))
        assert "error" in result

    def test_lists_branches(self, tmp_git_repo):
        result = forgegraph.branch_graph(str(tmp_git_repo))
...
class TestCrossRepoImpact:
    def test_empty(self):
        result = forgegraph.cross_repo_impact({})
        assert result == []
    ...
    def test_no_common_files(self, tmp_git_repo, tmp_path):
        ...
        result = forgegraph.cross_repo_impact({"repo-a": str(tmp_git_repo), "repo-b": str(repo_b)})
        assert result == []
...
class TestGenerateGraph:
    def test_repo_not_found(self, forge_config):
        result = forgegraph.generate_graph("no-such-repo")
        assert "error" in result
        assert "not found" in result["error"]

    def test_unknown_graph_type(self, forge_config, tmp_git_repo):
        ...
        cfg.save_config(c)
        result = forgegraph.generate_graph("my-repo", graph_type="invalid")
        assert "error" in result

    def test_co_change(self, forge_config, tmp_git_repo):
        import forge.config as cfg
        workspace_root = tmp_git_repo.parent.parent / "Workspace"
        workspace_root.mkdir(parents=True, exist_ok=True)
        c = forge_config.load_config()
        cfg.add_repo(c, {"name": "my-repo", "path": str(tmp_git_repo), "provider": "github", "url": "", "default_branch": "main"})
        cfg.save_config(c)
        result = forgegraph.generate_graph("my-repo", graph_type="co-change")
        assert "repo" in result
        assert result["repo"] == "my-repo"

    def test_branches(self, forge_config, tmp_git_repo):
        import forge.config as cfg
        workspace_root = tmp_git_repo.parent.parent / "Workspace"
        workspace_root.mkdir(parents=True, exist_ok=True)
        c = forge_config.load_config()
        cfg.add_repo(c, {"name": "my-repo", "path": str(tmp_git_repo), "provider": "github", "url": "", "default_branch": "main"})
        cfg.save_config(c)
        result = forgegraph.generate_graph("my-repo", graph_type="branches")
        assert "branches" in result
